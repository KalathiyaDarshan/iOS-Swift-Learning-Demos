...
class ProfileVC: BaseVC {
    ...
    @IBAction func onBtnBack(_ sender: UIButton) {
        ...
        if self.navigationScreen == .currentUserProfile || self.navigationScreen == .switchProfile {
            ...
            if !isVCFound {
                ...
                tabbar.selectedIndex = 2 // Select the desired tab index
        ...
        } else if self.navigationScreen == .missedOpportunities {
            
            if self.isProfileUnlockedSuccessfully {
                
                if let callback = self.unlockProfileCallback {
                    
                    if let objMissedOpportunityList = self.objMissedOpportunityList {
                        callback(objMissedOpportunityList)
                    }
                }
            }
    ...
    @IBAction func onBtnMenu(_ sender: UIButton) { // Work as share Button when open Profile screen from Interactions List
        ...
        switch self.navigationScreen {
            ...
            case .currentUserProfile, .discover, .QRScan, .switchProfile, .dynamicLink, .businessCardScan:
                ...
                if self.socialDetail?.user_mode == "0" || self.socialDetail?.subscriptionStatus == "0" {
                    ...
                    if let linkToShare = URL(string: self.socialDetail?.slug ?? "") {
                        let activityViewController = UIActivityViewController(activityItems: [linkToShare], applicationActivities: nil)
                        self.present(activityViewController, animated: true, completion: nil)
                    }
                    ...
                    if let linkToShare = URL(string: self.businessDetail?.slug ?? "") {
                        let activityViewController = UIActivityViewController(activityItems: [linkToShare], applicationActivities: nil)
                        self.present(activityViewController, animated: true, completion: nil)
                    }
                }
    ...
    @IBAction func onBtnQRCode(_ sender: UIButton) {
        ...
        switch self.navigationScreen {
            ...
            case .discover:
                let sendTokenVC = SendTokenVC.instantiate(fromAppStoryboard: .Discover)
                sendTokenVC.id = self.nearByUserDetail.id ?? ""
                
                if self.user_mode == "0" || self.subscriptionStatus == "0" {
                    sendTokenVC.profileURL = self.socialDetail?.profile_img ?? ""
                    sendTokenVC.firstName = self.socialDetail?.firstname ?? "A"
                    sendTokenVC.lastName = self.socialDetail?.lastname ?? "B"
                } else {
                    sendTokenVC.profileURL = self.businessDetail?.business_profileURL ?? ""
                    sendTokenVC.firstName = self.businessDetail?.business_firstName ?? "A"
                    sendTokenVC.lastName = self.businessDetail?.business_lastName ?? "B"
                }
                
                sendTokenVC.isFromProfile = true
            ...
            case .deviceHistory, .notification, .map:
                ...
                if !self.isUser_ReferalConnect {
                    ...
                    let sendTokenVC = SendTokenVC.instantiate(fromAppStoryboard: .Discover)
                    sendTokenVC.id = self.deviceScanHistory?.receiver_id ?? ""
                    sendTokenVC.profileURL = self.deviceScanHistory?.profile_url ?? ""
                    sendTokenVC.firstName = self.deviceScanHistory?.firstname ?? "A"
                    sendTokenVC.lastName = self.deviceScanHistory?.lastname ?? "B"
                    sendTokenVC.isFromProfile = true
                    self.navigationController?.pushViewController(sendTokenVC, animated: true)
                }
                break
            ...
            case .QRScan, .dynamicLink, .businessCardScan:
                ...
                if self.socialDetail?.user_mode == "0" || self.socialDetail?.subscriptionStatus == "0" {
                    sendTokenVC.id = self.socialDetail?.id ?? ""
                    sendTokenVC.profileURL = self.socialDetail?.profile_img ?? ""
                    sendTokenVC.firstName = self.socialDetail?.firstname ?? "A"
                    sendTokenVC.lastName = self.socialDetail?.lastname ?? "B"
                    
                } else {
                    sendTokenVC.id = self.businessDetail?.id ?? ""
                    sendTokenVC.profileURL = self.businessDetail?.business_profileURL ?? ""
                    sendTokenVC.firstName = self.businessDetail?.business_firstName ?? "A"
                    sendTokenVC.lastName = self.businessDetail?.business_lastName ?? "B"
                }
    ...
    @IBAction func onBtnBreakTheIce(_ sender: UIButton) {
        ...
        if self.navigationScreen == .QRScan || self.navigationScreen == .dynamicLink {
            ...
            let unlockProfileVC = UnlockProfileVC.instantiate(fromAppStoryboard: .Discover)
            unlockProfileVC.modalTransitionStyle = .crossDissolve
            unlockProfileVC.modalPresentationStyle = .overCurrentContext
            
            unlockProfileVC.objMissedOpportunityList = self.objMissedOpportunityList
            
            unlockProfileVC.unlockProfileCallback = { (isSuccess, msg) in
                
                if isSuccess {
                    
                    self.isProfileUnlockedSuccessfully = true
                    
                    self.imgViewBlurUserName.isHidden = true
                    self.lblUserName.isHidden = false
                    self.imgViewBlurCompanyOrUniversityName.isHidden = true
                    
                    self.lblCompanyOrUniversityName.isHidden = (self.lblCompanyOrUniversityName.text != nil && self.lblCompanyOrUniversityName.text != "") ? false : true
                    
                    self.imgViewBlurBluetoothStatus.isHidden = true
                    self.btnBluetoothStatus.isHidden = false
                    
                    self.imgViewAddEditProfile.isHidden = true
                    self.lblRemainingTime.isHidden = true
                    
                    self.btnBreakTheIce.isHidden = true
                    self.heightStackViewBreakTheIce.constant = 0.0
                    self.selectedButton = "Unlocked"
                    self.tblSocialNetwork.reloadData()
                    
                } else {
                    self.isProfileUnlockedSuccessfully = false
                }
            }
            
            self.present(unlockProfileVC, animated: true)
    ...
    private func loadDataFromAPI(isShowLoader: Bool = false, userMode: String = "") {
        ...
        switch self.navigationScreen {
            ...
            case .currentUserProfile, .switchProfile: // MARK: currentUserProfile | switchProfile
                ...
                } else if userMode == "1" { // Load Business Profile
                    
                    self.callGetBusinessProfileAPI(needToShowAlertForFailure: isShowLoader, UserID: UserLocalData.UserID) { isSuccess, response in
                        if let getBusinessProfileAPIResponse = response {
                            self.arrSocialNetwork.removeAll()
                            self.businessDetail = getBusinessProfileAPIResponse
                            self.viewAboutMe.isHidden = false
                            loginUser?.business_network = getBusinessProfileAPIResponse.business_network
                    ...
                    if isSuccess {
                        ...
                        if let getInfoAPIResponse = response {
                            if getInfoAPIResponse.user_mode == "0" || getInfoAPIResponse.subscriptionStatus == "0" {
                                ...
                                self.callGetBusinessProfileAPI(needToShowAlertForFailure: isShowLoader, UserID: self.nearByUserId) { isSuccess, response in
                                    if let getBusinessProfileAPIResponse = response {
                                        self.arrSocialNetwork.removeAll()
                                        self.businessDetail = getBusinessProfileAPIResponse
                    ...
                    if isSuccess {
                        ...
                        if let getInfoAPIResponse = response {
                            if getInfoAPIResponse.user_mode == "0" || getInfoAPIResponse.subscriptionStatus == "0" {
                                ...
                                self.callGetBusinessProfileAPI(needToShowAlertForFailure: isShowLoader, UserID: getInfoAPIResponse.id ?? "") { isSuccess, response in
                                    if let getBusinessProfileAPIResponse = response {
                                        self.arrSocialNetwork.removeAll()
                                        self.businessDetail = getBusinessProfileAPIResponse
                                        self.viewAboutMe.isHidden = false
            ...
            case .missedOpportunities: // MARK: missedOpportunities
    ...
    private func setupUI() {
        ...
        switch self.navigationScreen {
            ...
            case .discover, .QRScan, .dynamicLink, .businessCardScan:
        ...
        switch self.navigationScreen {
            ...
            case .currentUserProfile, .switchProfile:
                ...
                if UserLocalData.userMode == "0" {
                    ...
                    let dbUserData = DBManager.checkUserBusinessInfoExist(userID: UserLocalData.UserID)
                    
                    if let dbUserModel = dbUserData.userData, dbUserData.isSuccess {
                        
                        self.businessDetail = dbUserModel
                        self.viewAboutMe.isHidden = false
                        loginUser?.business_network = dbUserModel.business_network
                        
                        // TODO: For Subscription Status
                        // if self.businessDetail?.subscriptionStatus == "1" {
                        //     self.imgViewSubscriptionStatus.isHidden = false
                        // } else if self.businessDetail?.subscriptionStatus == "0" {
                        //     self.imgViewSubscriptionStatus.isHidden = true
                        // }
                        
                        self.imgViewAddEditProfile.isHidden = false
                        
                        if let businessProfileImage = self.businessDetail?.business_profileURL {
                        ...
                        self.setUserBusinessDetail()
                        self.loadDataFromAPI(isShowLoader: false, userMode: "1")
                        
                    } else {
                        
                        self.loadDataFromAPI(isShowLoader: true, userMode: "1")
                    }
                }
                break
            ...
            case .discover:
                ...
                } else { // Load Business Profile
                    
                    let dbUserData = DBManager.checkUserBusinessInfoExist(userID: self.nearByUserId)
            ...
            case .deviceHistory, .map, .notification:
                ...
                if self.deviceScanHistory != nil { // .deviceHistory
                    ...
                    if dbUserData.userData != nil {
                        ...
                        } else if dbUserData.userData?.subscriptionStatus == "1" && dbUserData.userData?.user_mode == "1" {
                            
                            let dbUserData = DBManager.checkUserBusinessInfoExist(userID: self.deviceScanHistory?.receiver_id ?? "")
            ...
            case .missedOpportunities:
                ...
                if self.selectedButton == "Locked" {
                    
                    self.imgViewBlurUserName.isHidden = false
                    self.lblUserName.isHidden = true
                    self.imgViewBlurBluetoothStatus.isHidden = false
                    self.btnBluetoothStatus.isHidden = true
                    
                    self.imgViewAddEditProfile.isHidden = false
                    self.imgViewAddEditProfile.image = UIImage(named: "ic_remaining_time")
                    self.lblRemainingTime.isHidden = false
                    
                    // Check if the string ends with the specified suffix
                    if self.timeRemainingToUnlockProfile.hasSuffix(" remaining") {
                        // Remove the suffix from the string
                        self.timeRemainingToUnlockProfile = self.timeRemainingToUnlockProfile.replacingOccurrences(of: " remaining", with: "")
                        print("Suffix removed:", self.timeRemainingToUnlockProfile)
                    } else {
                        print("String does not end with the specified suffix")
                    }
                    
                    self.lblRemainingTime.text = self.timeRemainingToUnlockProfile
                    
                } else if self.selectedButton == "Unlocked" {
                    
                    self.imgViewAddEditProfile.isHidden = true
                    self.lblRemainingTime.isHidden = true
                }
                
                if self.objMissedOpportunityList != nil { // .deviceHistory
                    
                    let dbUserData = DBManager.checkUserSocialInfoExist(userID: self.objMissedOpportunityList?.receiver_id ?? "")
                    ...
                    if dbUserData.userData != nil {
                        ...
                        if dbUserData.userData?.user_mode == "0" || dbUserData.userData?.subscriptionStatus == "0" {
                            ...
                            let dbUserData = DBManager.checkUserBusinessInfoExist(userID: self.objMissedOpportunityList?.receiver_id ?? "")
    ...
    //    private func updateCurrentUserProfileInDB(currentUserData: UserDetail) {
    //
    //        DispatchQueue.global(qos: .background).async {
    //
    //            if DBManager.checkUserSocialInfoExist(userID: currentUserData.id!).isSuccess {
    //
    //                let isSocialProfileUpdated = DBManager.updateSocialProfile(userID: currentUserData.id!, requestBody: currentUserData.toJSONString()!)
    //                print("isSocialProfileUpdated :: \(isSocialProfileUpdated)")
    //
    //            } else {
    //
    //                let isUserDataInsterted = DBManager.insertUserData(userID: currentUserData.id!, requestBody: currentUserData.toJSONString()!, businessProfile: "")
    ...
    private func getProfileLink() {
        ...
        if self.socialDetail?.user_mode == "0" || self.socialDetail?.subscriptionStatus == "0" {
            ...
            let link = URL(string: self.socialDetail?.slug ?? "")
            self.shareUserProfile = link
            
        } else {
            
            let link = URL(string: self.businessDetail?.slug ?? "")
            self.shareUserProfile = link
        }
    }
...
extension ProfileVC: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let cell = self.tblSocialNetwork.dequeueReusableCell(withIdentifier: SocialNetworkTblCell.identifier) as? SocialNetworkTblCell {
            ...
            if self.navigationScreen == .missedOpportunities && self.selectedButton == "Locked" {
                
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.3) {
                    let blurView = DynamicBlurView(frame: cell.viewBlur.bounds)
                    blurView.backgroundColor = UIColor.appWhite_FFFFFF_Opacity_16()
                    blurView.blurRadius = 1
                    blurView.viewCornerRadius = 14
                    blurView.blurRatio = 4
                    blurView.layer.cornerRadius = 0
                    blurView.blendMode = .clear
                    blurView.trackingMode = .none
                    blurView.clipsToBounds = true
                    cell.viewBlur.clipsToBounds = true
                    cell.viewBlur.addSubview(blurView)
        ...
        if let socialDetail = self.socialDetail {
            ...
            if socialDetail.profession_type == "1" { // Public
                ...
                if socialDetail.company_name != nil && socialDetail.company_name != "" {
                    ...
                    if self.navigationScreen == .missedOpportunities {
                        if self.selectedButton == "Locked" {
                            self.imgViewBlurCompanyOrUniversityName.isHidden = false
            ...
            } else if socialDetail.profession_type == "2" { // Student
                
                if socialDetail.university != nil && socialDetail.university != "" {
                    self.lblCompanyOrUniversityName.text = socialDetail.university
            ...
            // self.arrSocialNetwork.sort { $0.social_app_order ?? "" < $1.social_app_order ?? "" }
            ...
            if self.navigationScreen == .currentUserProfile || self.navigationScreen == .deviceHistory || self.navigationScreen == .map || self.navigationScreen == .notification || self.navigationScreen == .switchProfile || self.navigationScreen == .missedOpportunities { // MARK: currentUserProfile || deviceHistory || map || notification || switchProfile || missedOpportunities
            ...
            if self.navigationScreen == .currentUserProfile || self.navigationScreen == .switchProfile || self.navigationScreen == .businessCardScan { // MARK: currentUserProfile || switchProfile
            ...
            } else if self.navigationScreen == .missedOpportunities { // MARK: missedOpportunities
                ...
                if self.selectedButton == "Locked" {
                    ...
                    self.btnBreakTheIce.isHidden = false
                    self.heightStackViewBreakTheIce.constant = 48.0
                    self.btnBreakTheIce.setTitle("Unlock", for: .normal)
...
extension ProfileVC {
    ...
    private func setUserBusinessDetail() {
        ...
        if let businessDetail = self.businessDetail {
            ...
            self.setBackgroundViewForBusinessProfile()
            
            // MARK: - COMMON -
            // MARK: ------------------------------Profile Image---------------------------------
            
            if let url = URL(string: businessDetail.business_profileURL ?? "") {
                
                self.imgViewProfile.af_setImage(withURL: url)
                
            } else {
                
                if let firstName = businessDetail.business_firstName?.capitalized.first {
                    self.businessFirstname = String(firstName)
                    
                } else if let firstName = businessDetail.firstname?.capitalized.first {
                    self.businessFirstname = String(firstName)
                }
                
                if let lastName = businessDetail.business_lastName?.capitalized.first {
                    self.businessLastname = String(lastName)
                    
                } else if let lastName = businessDetail.lastname?.capitalized.first {
                    self.businessLastname = String(lastName)
                }
                
                self.imgViewProfile.image = UIImage.imageWithInitial(initial: "\(self.businessFirstname)\(self.businessLastname)", imageSize: self.imgViewProfile.bounds.size, gradientColors: [UIColor.appWhite_FFFFFF_Opacity_16(), UIColor.appWhite_FFFFFF_Opacity_16()], font: UIFont(name: "RedHatDisplay-Medium", size: 16) ?? UIFont.boldSystemFont(ofSize: 16))
            }
            
            // MARK: ------------------------------User Name-------------------------------------
            
            let businessUserName = (businessDetail.business_firstName?.capitalizingFirstLetter() ?? "") + " " + (businessDetail.business_lastName?.capitalizingFirstLetter() ?? "")
            let socialUserName = (businessDetail.firstname?.capitalizingFirstLetter().trime() ?? "") + " " + (businessDetail.lastname?.capitalizingFirstLetter().trime() ?? "")
            self.lblUserName.text = businessUserName.trime() == "" ? socialUserName : businessUserName
            
            // MARK: ------------------------------Company | University--------------------------
            
            if businessDetail.profession_type == "1" { // Public
                
                if businessDetail.business_company != nil && businessDetail.business_company != "" {
                    self.lblCompanyOrUniversityName.text = businessDetail.business_company
            ...
            } else if businessDetail.profession_type == "2" { // Student
                
                if businessDetail.business_university != nil && businessDetail.business_university != "" {
                    self.lblCompanyOrUniversityName.text = businessDetail.business_university
            ...
            if businessDetail.caption != nil {
                self.btnBluetoothStatus.setTitle(businessDetail.caption, for: .normal)
            } else {
                self.btnBluetoothStatus.setTitle("No Caption", for: .normal)
            }
            ...
            self.lblUserProfileViewCount.text = businessDetail.view_counts ?? "0"
            self.lblLinkClicksCount.text = businessDetail.linksTapped ?? "0"
            self.lblUserInteractionCount.text = businessDetail.interactions ?? "0"
            self.lblUserToken.text = "\(businessDetail.totalBST ?? 0)"
            
            // MARK: ------------------------------Bio-------------------------------------------
            
            if self.navigationScreen == .currentUserProfile || self.navigationScreen == .switchProfile {
                self.lblBio.text = businessDetail.business_bio != "" && businessDetail.business_bio != nil ? businessDetail.business_bio : "You have not added this information yet"
            } else {
                self.lblBio.text = businessDetail.business_bio != "" && businessDetail.business_bio != nil ? businessDetail.business_bio : "No information has been added"
            }
            
            if businessDetail.business_bio == "" || businessDetail.business_bio == nil { self.lblBio.textColor = UIColor.appGray_98A2B1() }
            self.viewBlur.isHidden = true
            
            // MARK: ------------------------------Interest Topics-------------------------------
            
            self.arrInterestTopic = businessDetail.userInterest ?? [User_Interest]()
            ...
            if let businessNetwork = businessDetail.business_network {
                for index in 0 ..< businessNetwork.count {
                    if let socialNetworkList = businessNetwork[index].social_network_list {
                        for i in 0 ..< socialNetworkList.count {
                            if let value = socialNetworkList[i].value, !value.isEmpty {
                                self.arrSocialNetwork.append(socialNetworkList[i])
            ...
            if self.navigationScreen == .currentUserProfile || self.navigationScreen == .deviceHistory || self.navigationScreen == .map || self.navigationScreen == .notification || self.navigationScreen == .switchProfile || self.navigationScreen == .missedOpportunities { // MARK: currentUserProfile || deviceHistory || map || notification || switchProfile || missedOpportunities
                ...
                if businessDetail.business_private_mode == "0" {
                ...
                } else if businessDetail.business_private_mode == "1" {
            ...
            /*
             ...
             if let privateModeList = businessDetail.business_private_mode_list {
